...
class Database:
    ...
    def update_transcription(self, task_id: str, **kwargs) -> bool:
        ...
        with self.get_session() as session:
            transcription = session.query(Transcription).filter_by(task_id=task_id).first()
            if not transcription:
                return False
            
            # Update allowed fields
            allowed_fields = {
                'audio_path', 'srt_path', 'model', 'language', 'status',
                'progress', 'current_step', 'error_message', 'result',
                'started_at', 'completed_at', 'estimated_completion_time',
                'extra_metadata'
            }
    ...
    def get_disk_usage_stats(self) -> Dict[str, Any]:
        ...
        with self.get_session() as session:
            transcriptions = session.query(
                Transcription.audio_path,
                Transcription.srt_path
            ).filter(
                (Transcription.audio_path != None) | (Transcription.srt_path != None)
            ).all()
    ...
    def _model_to_dict(self, transcription: Transcription) -> Dict[str, Any]:
        ...
        return {
            ...
            'estimated_completion_time': transcription.estimated_completion_time.isoformat() if transcription.estimated_completion_time else None,
