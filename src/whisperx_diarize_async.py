...
def whisperx_diarize_with_progress(
    ...
    command += f" --diarize --min_speakers=2 --max_speakers=4"
    command += f" --chunk_size {chunk_size}"
    command += f" --compute_type float32"
    
    if hug_token:
        command += f" --hf_token {hug_token}"
    
    # whisper parameters
    command += f" --temperature 0.1"
    command += f" --fp16 False"
    command += f" --language {language}"
    
    if initial_prompt:
        command += f" --initial_prompt '{initial_prompt}'"
    
    command += f" --condition_on_previous_text False"
    command += f" --output_dir '{output_dir}'"
    command += f" --output_format srt"
    command += f" --print_progress True"
    ...
    for line in iter(process.stdout.readline, ''):
        ...
        if progress_match:
            ...
            if current_step in steps_progress:
                ...
                current_progress = min_prog + int((max_prog - min_prog) * percentage / 100)
        ...
        if segment_match:
            ...
            if current_step == 'transcribing' and total_segments > 0:
                min_prog, max_prog, _ = steps_progress[current_step]
                current_progress = min_prog + (max_prog - min_prog) * current_segment // total_segments
